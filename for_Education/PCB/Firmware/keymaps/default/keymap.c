...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(
		...
		KC_PGUP, KC_TAB, KC_Q, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_LBRC, KC_RBRC, KC_BSPC, KC_END, 
		...
		MO(1), KC_LGUI, KC_LALT, KC_SPC, LT(1, KC_MPLY), LT(1, KC_RALT), LT(2, KC_RCTL), KC_LEFT, KC_DOWN, KC_RGHT
    ),
    [1] = LAYOUT(
...
void keyboard_post_init_user(void) {
    gpio_set_pin_output(LAYER_INDICATOR_PIN);
}

layer_state_t layer_state_set_user(layer_state_t state) {
    if (layer_state_cmp(state, LAYER_LOCK_LAYER)) {
        gpio_write_pin_high(LAYER_INDICATOR_PIN);
    } else {
        gpio_write_pin_low(LAYER_INDICATOR_PIN);
    }
    return state;
}
